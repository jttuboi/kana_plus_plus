...
void main() {
  group('AppRepository', () {
    ...
    test('gets first time', () {
      when(() => mockDatabase.load(BoxTag.app)).thenAnswer((invocation) => Future.value());
      when(() => mockDatabase.get(DatabaseTag.firstTime, defaultValue: Default.firstTime)).thenAnswer((invocation) => Future.value(true));
    ...
    test('puts first time', () async {
      when(() => mockDatabase.load(BoxTag.app)).thenAnswer((invocation) => Future.value());
      when(() => mockDatabase.put(DatabaseTag.firstTime, true)).thenAnswer((invocation) => Future.value());
