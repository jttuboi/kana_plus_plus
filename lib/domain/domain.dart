// ignore: depend_on_referenced_packages
export 'package:flutter_gen/gen_l10n/j_strings.dart';

export 'models/kana_model.dart';
export 'models/kana_stats.dart';
export 'models/kana_type.dart';
export 'models/point_stats.dart';
export 'models/stroke_stats.dart';
export 'models/training_stats.dart';
export 'models/translate_model.dart';
export 'models/word_model.dart';
export 'models/word_stats.dart';
export 'repositories/app_interface_repository.dart';
export 'repositories/kana_type_interface_repository.dart';
export 'repositories/language_interface_repository.dart';
export 'repositories/quantity_of_words_interface_repository.dart';
export 'repositories/show_hint_interface_repository.dart';
export 'repositories/statistics_interface_repository.dart';
export 'repositories/words_interface_repository.dart';
export 'services/info_getter_interface.dart';
export 'services/kana_checker_interface.dart';
export 'services/rate_launcher_interface.dart';
export 'services/share_launcher_interface.dart';
export 'services/stroke_reducer_interface.dart';
export 'services/url_launcher_interface.dart';
export 'services/word_to_kana_converter_interface.dart';
export 'utils/banner_url.dart';
export 'utils/consts.dart';
export 'utils/icon_url.dart';
export 'utils/image_url.dart';
